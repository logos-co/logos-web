/**
 * The MIME types a Logos Storage node will accept on upload.
 *
 * The node does not take any string. It looks the Content-Type up in nim's
 * `std/mimetypes` table and answers 422 if nothing maps to it:
 *
...
/**
 * Whether a node would accept this Content-Type.
 *
 * An empty type is fine: the node treats a missing Content-Type as no mimetype
 * at all rather than an error, and simply leaves the field out of the manifest.
 */
export function isAcceptedMimetype(mimetype: string): boolean {
  ...
  return ACCEPTED.has(mimetype.split(';')[0].trim().toLowerCase())
}
