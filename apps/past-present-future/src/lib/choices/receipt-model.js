...
export const moneyS = (n) => (n < 0 ? '−£' : '+£') + Math.abs(n).toLocaleString('en-GB');

const row = (label, amount, cls = '') => ({ t: 'row', label, amount, cls });
const rule = { t: 'rule' };

const letterhead = (meta) => [{ t: 'brand' }, { t: 'title' }, { t: 'meta', text: meta }, rule];
...
export function chapterReceipt(scene, option, ledger, debtTotal) {
	...
	const lines = letterhead(`No. ${scene.tag} · Age ${scene.age}–${SCENES[SCENES.findIndex((s) => s.age === scene.age) + 1]?.age ?? 45}`);
	if (b.inc != null) lines.push(row(b.incLabel || 'Take-home pay', `£${b.inc.toLocaleString('en-GB')} /mo`, 'pos'));
	...
	for (const r of monthly) lines.push(row(r[0], `−£${r[1].toLocaleString('en-GB')}${b.inc != null ? ' /mo' : ''}`));
	if (b.inc != null) {
		const costs = monthly.reduce((a, r) => a + r[1], 0);
		const net = b.inc - costs;
		lines.push(row('Left each month', moneyS(net), `sep ${net >= 0 ? 'pos' : 'neg'}`));
		for (const r of lumps) lines.push(row(r[0], `−£${r[1].toLocaleString('en-GB')}${r[2] === 'total' ? ' once' : ''}`));
	}
	lines.push(row(b.footLabel, moneyS(footMoney), `foot ${footMoney >= 0 ? 'pos' : ''}`));
	lines.push(rule);
...
export function finalReceipt(ledger, debtTotal) {
	...
	if (srcs.length) lines.push({ t: 'sources', items: srcs.map((name) => ({ name, url: SRC_URLS[name] })) });
	return lines;
}
