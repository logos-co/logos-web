...
export function printLineSfx() {
	if (!actx) return;
	const t = actx.currentTime, dur = 0.07;
	...
	o.connect(og);
	og.connect(master);
	const bs = Math.floor(actx.sampleRate * dur), buf = actx.createBuffer(1, bs, actx.sampleRate), d = buf.getChannelData(0);
	...
	o.start(t); lfo.start(t); nz.start(t); low.start(t);
	o.stop(t + dur + 0.02); lfo.stop(t + dur + 0.02); nz.stop(t + dur + 0.02); low.stop(t + 0.06);
}
