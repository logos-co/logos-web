...
`apps/logos-crm` holds a PostgreSQL-backed vertical slice: a pipeline summary, a Kanban board and searchable case table over per-team pipelines, case detail/next-action view, case creation, status progression, stage moves, people and organisation directories, contact methods, explicit case relationships, record activity timelines, and assigned tasks through Next.js Route Handlers. Activities and tasks each belong to exactly one case, person, or organisation, with that invariant enforced in PostgreSQL. It supports recording notes, creating tasks, and completing or reopening tasks.
