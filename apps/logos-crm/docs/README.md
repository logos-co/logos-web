...
`apps/logos-crm` holds a PostgreSQL-backed vertical slice: a pipeline summary, a Kanban board and searchable case table over per-team pipelines, a merged record timeline with rich text notes, case detail/next-action view, case creation, status progression, stage moves, people and organisation directories, contact methods, explicit case relationships, record activity timelines, and assigned tasks through Next.js Route Handlers. Activities and tasks each belong to exactly one case, person, or organisation, with that invariant enforced in PostgreSQL. It supports recording notes, creating tasks, and completing or reopening tasks.

Every record opens on one chronological history that merges notes, contact, tasks, stage and status changes, ownership, and the decision. Those facts existed before and were each shown somewhere else, so "what has happened with this partner" was a question you answered by reading three panels and inferring the order between them. The timeline is a read model over the same tables - nothing writes to it - so it can never disagree with the records it summarises. Notes are Markdown and stay editable from the feed by their author, with the previous text kept in the audit event and deleted notes keeping their place without their body.

Fields that pick a record - the organisation on a case, its owner, its primary contact - use a searchable combobox rather than a native select. The list filters as you type and, where creating is allowed, Enter turns what you typed into a new record without leaving the form. That matters most for organisations: a lead whose organisation is not in the CRM yet is the common case, and a round trip through the directory is how leads end up recorded nowhere.

List endpoints are capped rather than paginated (`LIST_LIMIT_DEFAULT`), and substring search is backed by `pg_trgm` GIN indexes, because every search path here is `ILIKE '%term%'` and a B-tree cannot serve one. Both are holding measures sized for the Notion import; a real pager needs a cursor and belongs with that work.

Cases run on a named pipeline. Ecodev and Movement carry the stage catalogues taken from the production `IFT BD CRM` Notion export, which keeps a separate status column per business unit; the two vocabularies are disjoint, so a stage is only ever valid on its own pipeline and that pairing is enforced at every write. The board groups by stage, offers a move as either a drag or a per-card select, and writes the same workflow history and audit event a status change does. `contracts/pipeline.ts` is the catalogue; changing it is a code change, which is deliberate - stage names are what every report is counted by. It does not yet have production authentication, CASL enforcement, Graphile Worker jobs, reminders, notifications, imports, merges, or the intake cutover.
