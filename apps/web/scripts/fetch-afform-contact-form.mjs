...
const FORM_NAME = FORM_IDX !== -1 ? args[FORM_IDX + 1] : "afformCircleContactForm";
...
async function civicrmApi(entity, action, body = {}) {
  ...
  if (!res.ok) throw new Error(`${entity}/${action}: ${res.status} ${await res.text()}`);
  return res.json();
}
...
const FALLBACKS_PATH = path.join(root, "lib", "civicrm", "connect-fallbacks.json");
...
async function fetchOptionValuesForGroup(gid) {
  ...
  if (values.length > 100) {
    console.log(`        ⚠ Got ${values.length} values (API likely ignored where clause). Filtering to group ${gid}...`);
...
async function fetchAfformOptionLists(knownGroupIds, knownOptions) {
  ...
  try {
    ...
    const customFields = (customRes.values || []).filter((cf) => cf.option_group_id != null);
    console.log(`        → ${customRes.values?.length ?? 0} custom fields, ${customFields.length} with option_group_id`);
    ...
    console.log(`        API: role=${apiRoleGid}, skills=${apiSkillsGid} | Using: role=${roleGid}, skills=${skillsGid}`);
  } catch (e) {
  ...
  if (chatServiceGid == null) {
    try {
      ...
      const providerField = (imFieldsRes.values || []).find((f) => f.name === "provider_id");
      if (providerField?.option_group_id != null) {
        chatServiceGid = normOptionGroupId(providerField.option_group_id);
        console.log(`        InstantMessenger.getFields → chatServiceGid=${chatServiceGid}`);
      }
    } catch (e) {
  ...
  if (chatServiceGid != null) {
    ...
    console.log("        No chatServiceGid resolved. Will use fallback options.");
  }

  // Fetch OptionValues for each resolved group
  console.log("  [3/3] OptionValue.get...");
  const toFetch = { role: roleGid, skills: skillsGid, chatService: chatServiceGid };
  for (const [key, gid] of Object.entries(toFetch)) {
    if (gid == null) {
      console.log(`        ${key}: no group ID, using known options (${knownOptions[key]?.length ?? 0})`);
      if (knownOptions[key]?.length) options[key] = knownOptions[key];
      continue;
    }
    try {
      const vals = await fetchOptionValuesForGroup(gid);
      console.log(`        ${key} (group ${gid}): ${vals.length} options (${vals.map((o) => o.label).join(", ")})`);
      if (vals.length > 0) {
        options[key] = vals;
      } else if (knownOptions[key]?.length) {
        console.log(`        ${key}: API returned 0 values, keeping known options (${knownOptions[key].length})`);
...
const FIELD_TO_FORM_KEY = [
  [{ name: "Contacts_Custom_Fields.Role", join: null }, { formKey: "role", inputType: "select" }],
  [{ name: "first_name", join: null }, { formKey: "name", inputType: "text" }],
  [{ name: "city", join: "Address" }, { formKey: "city", inputType: "text" }],
  [{ name: "country_id", join: "Address" }, { formKey: "country", inputType: "select", label: "Country" }],
  [{ name: "Skills_Socials.Skills_Experience", join: null }, { formKey: "skills", inputType: "select" }],
  [{ name: "email", join: "Email" }, { formKey: "email", inputType: "email" }],
  [{ name: "Skills_Socials.Affiliated_Organisations", join: null }, { formKey: "affiliatedOrgs", inputType: "text" }],
  [{ name: "url", join: "Website" }, { formKey: "website", inputType: "text" }],
  [{ name: "name", join: "IM" }, { formKey: "chat", inputType: "text" }],
  [{ name: "provider_id", join: "IM" }, { formKey: "chatService", inputType: "select" }],
  [{ name: "note", join: "Note" }, { formKey: "openText", inputType: "textarea" }],
  [{ name: "Skills_Socials.Informed_about_events_in_my_city", join: null }, { formKey: "wantsEvents", inputType: "checkbox" }],
  [{ name: "Skills_Socials.Subscribed_to_Logos_Newsletter", join: null }, { formKey: "wantsNewsletter", inputType: "checkbox" }],
  [{ name: "Skills_Socials.Tell_us_a_bit_about_your_background_the_kinds_of_things_you_like", join: null }, { formKey: "backgroundBuilder", inputType: "textarea" }],
  [{ name: "Skills_Socials.What_types_of_technologies_do_you_envision_can_help_your_local_c", join: null }, { formKey: "techVision", inputType: "textarea" }],
  [{ name: "Skills_Socials.Tell_us_a_bit_about_your_background_what_kind_of_community_or_lo", join: null }, { formKey: "backgroundLeader", inputType: "textarea" }],
  [{ name: "Skills_Socials.What_types_of_organized_activities_do_you_envision_doing_to_impa", join: null }, { formKey: "activitiesVision", inputType: "textarea" }],
  [{ name: "Skills_Socials.Tell_us_about_your_organization_community_or_project_what_proble", join: null }, { formKey: "backgroundPartner", inputType: "textarea" }],
  [{ name: "Skills_Socials.What_else_would_you_like_us_to_know_What_questions_do_you_have_f", join: null }, { formKey: "questions", inputType: "textarea" }],
  [{ name: "Circle_Case.Profile", join: null }, { formKey: "caseProfile", inputType: "hidden" }],
  [{ name: "Circle_Case.Lead_Source", join: null }, { formKey: "caseLeadSource", inputType: "hidden" }],
];
...
function getFormKeyAndType(fieldName, join, defn = {}) {
  const entry = FIELD_TO_FORM_KEY.find(([key]) => key.name === fieldName && key.join === join);
  if (entry) {
  ...
  const inputType = defn.input_type === "TextArea" || (defn.label && defn.label.length > 80) ? "textarea" : "text";
  return { formKey: key, inputType };
}

function extractFields(children, context = { entity: "Individual1", join: null }) {
  const fields = [];
  ...
  for (const node of children) {
    ...
    if (node["#tag"] === "af-field") {
      ...
      const fieldMeta = {
        entity: context.entity, join: context.join, fieldName: node.name,
        label: mappedLabel || defn.label || node.name,
        required: !!defn.required,
        options: defn.options ?? null, inputAttrs: defn.input_attrs ?? [],
    ...
    if (node["#tag"] === "div" && node["af-join"]) {
      fields.push(...extractFields(node["#children"] ?? [], { ...context, join: node["af-join"] }));
      continue;
    }
    if (node["af-fieldset"] && node["#children"]) {
      fields.push(...extractFields(node["#children"], { ...context, entity: node["af-fieldset"] }));
      continue;
    }
    if (node["#children"]) {
...
function getTextFromChildren(children) {
  ...
  return children.map((c) => (c?.["#text"] ?? "")).filter(Boolean).join("").trim();
}
...
function getTextRecursive(children) {
  ...
  return parts.join(" ").trim().replace(/\s*&nbsp;\s*/gi, " ").replace(/\s+/g, " ").trim();
}
...
function findFirstTextInLayout(children, tag) {
  ...
  for (const node of children) {
    if (!node || typeof node !== "object") continue;
    if (node["#tag"] === tag) return getTextFromChildren(node["#children"] ?? []);
    if (node["#children"]) {
...
function findAllTextByTag(children, tag, out = []) {
  ...
  for (const node of children) {
    if (!node || typeof node !== "object") continue;
    if (node["#tag"] === tag) out.push(getTextFromChildren(node["#children"] ?? []));
...
function findTextByClass(children, className) {
  ...
  for (const node of children) {
    if (!node || typeof node !== "object") continue;
    if (String(node.class ?? "").includes(className)) return getTextRecursive(node["#children"] ?? []);
    if (node["#children"]) {
...
function findFirstLinkHrefByClass(children, className) {
  ...
  for (const node of children) {
    if (!node || typeof node !== "object") continue;
    if (String(node.class ?? "").includes(className)) return findFirstLinkHref(node["#children"] ?? []) || "";
    if (node["#children"]) {
...
function extractPageCopy(layout) {
  const result = { pageHeading: "", pageIntro: "", pagePrivacy: "", pagePrivacyLink: "" };
...
async function syncConnectFallbacks(afformOptions) {
  ...
  const mergedOptions = {
    ...fallbacks.options,
    role: afformOptions.role?.length ? afformOptions.role : (fallbacks.options?.role ?? []),
    skills: afformOptions.skills?.length ? afformOptions.skills : (fallbacks.options?.skills ?? []),
    chatService: afformOptions.chatService?.length ? afformOptions.chatService : (fallbacks.options?.chatService ?? []),
    country: afformOptions.country?.length ? afformOptions.country : (fallbacks.options?.country ?? []),
  };
...
function validateOutput(out, options) {
  ...
  for (const key of CORE_FORM_KEYS) {
    if (!parsedKeys.includes(key)) warnings.push(`Missing core formKey "${key}"`);
  }
  if (!out.pageHeading) warnings.push("pageHeading is empty");
  if (!options.skills?.length && parsedKeys.includes("skills")) warnings.push("skills options are empty");
  if (!options.chatService?.length && parsedKeys.includes("chatService")) warnings.push("chatService options are empty");
...
async function processForm(form, afformOptions) {
  ...
  if (!layout || !Array.isArray(layout)) throw new Error("No layout array on form");
  ...
  const { pageHeading, pageIntro, pagePrivacy, pagePrivacyLink } = extractPageCopy(layout);

  const out = {
    formName: form.name,
    title: form.title,
    serverRoute: form.server_route,
    confirmationMessage: form.confirmation_message ?? "Thank you for registering, we'll be in touch soon.",
  ...
  console.log(`  formKeys:     ${allFields.map((f) => f.formKey).filter(Boolean).join(", ")}`);
  console.log(`  pageHeading:  ${out.pageHeading}`);
  console.log(`  pageIntro:    ${out.pageIntro.slice(0, 80)}${out.pageIntro.length > 80 ? "..." : ""}`);
  console.log(`  pagePrivacy:  ${out.pagePrivacy.slice(0, 80)}${out.pagePrivacy.length > 80 ? "..." : ""}`);
  console.log(`  privacyLink:  ${out.pagePrivacyLink}`);
  console.log(`\n--- Options summary ---`);
  console.log(`  role:        ${afformOptions.role?.length ?? 0} (${afformOptions.role?.map((o) => o.label).join(", ") || "none"})`);
  console.log(`  skills:      ${afformOptions.skills?.length ?? 0}`);
  console.log(`  chatService: ${afformOptions.chatService?.length ?? 0} (${afformOptions.chatService?.map((o) => o.label).join(", ") || "none"})`);
  ...
  const slug = form.name.replace(/^afform/, "").replace(/([A-Z])/g, "-$1").toLowerCase().replace(/^-/, "");
...
`;
  ...
  if (DRY_RUN) {
    ...
    try {
      ...
      console.log(existing === jsContent ? "[dry-run] No changes" : "[dry-run] File WOULD change");
    } catch { console.log("[dry-run] File is new"); }
...
async function fetchFormFromApi() {
  ...
  const res = await civicrmApi("Afform", "get", {
    ...
    select: [
      "name", "title", "server_route", "layout",
      "confirmation_type", "confirmation_message",
      "submit_enabled", "create_submission",
    ],
  });
  ...
  if (!form) {
    ...
    console.error(`Forms returned: ${allForms.map((f) => f.name).filter(Boolean).join(", ")}`);
    process.exit(1);
  }
...
async function loadFormFromFile(filePath) {
  ...
  if (!form) {
    ...
    console.error(`Forms found: ${list.map((f) => f?.name).filter(Boolean).join(", ")}`);
    process.exit(1);
  }
  return form;
}
...
async function main() {
  ...
  console.log(`Known option group IDs from connect-fallbacks.json: ${JSON.stringify(knownGroupIds)}`);
  console.log(`Known options: role=${knownOptions.role?.length ?? 0}, skills=${knownOptions.skills?.length ?? 0}, chatService=${knownOptions.chatService?.length ?? 0}`);
  ...
  if (!CIVICRM_BASE_URL || !CIVICRM_API_KEY) {
    ...
    console.error("Set them in apps/civi-crm/.env.local (or apps/web/.env.local) and run: pnpm fetch:afform-contact");
    process.exit(1);
  }
