...
function HoverStackItem({
  ...
  return (
    <div
      className={`group/stack-item relative flex items-center justify-center overflow-hidden rounded-3xl border border-brand-dark-green px-6 text-center text-brand-dark-green transition-[border-color] duration-200 ease-out hover:border-brand-dark-green/30 ${className}`}
    >
      ...
      <Button
        ...
        className={`absolute top-2.5 right-2.5 z-10 cursor-pointer transition-opacity duration-200 ease-out md:top-3 md:right-3 ${
          ctaVisibleByDefault
            ? ''
            : 'pointer-events-none opacity-0 group-hover/stack-item:pointer-events-auto group-hover/stack-item:opacity-100 focus-visible:pointer-events-auto focus-visible:opacity-100'
        }`}
      >
        {ctaLabel}
      </Button>
      ...
      <div
        className={`relative z-[1] flex max-w-[222px] flex-col items-center gap-3 transition-transform duration-200 ease-out ${
          hasDetails ? 'md:group-hover/stack-item:-translate-y-8' : ''
        }`}
      >
        <span
