...
function buildSrcDoc(
  ...
  const heightScript = `<script>(function(){var frameId=${JSON.stringify(frameId)};var last=-1;function post(){var height=Math.max(document.documentElement.scrollHeight||0,document.body?document.body.scrollHeight:0);if(height===last)return;last=height;parent.postMessage({type:'logos-media-embed-height',frameId:frameId,height:height},'*');}window.addEventListener('load',post);window.addEventListener('resize',post);new MutationObserver(post).observe(document.documentElement,{childList:true,subtree:true,attributes:true,characterData:true});if(typeof ResizeObserver!=='undefined')new ResizeObserver(post).observe(document.documentElement);setTimeout(post,120);setTimeout(post,600);})();</script>`
