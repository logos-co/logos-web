...
function HeroAnchorLink({ children, href }: { children: ReactNode; href: string }) {
  return (
    <a
      href={href}
  ...
  return (
    <main className="bg-brand-off-white">
      <section className="relative h-[483px] px-3 pt-6 text-brand-dark-green md:h-[487px]">
        ...
        <p className="text-mono-s absolute top-6 left-[203px] flex w-[178px] items-center gap-3 md:left-1/2 md:w-[226px] md:translate-x-[6px]">
          <LogosMark size={10} />
          {t('hero.eyebrow')}
        </p>
        ...
        <h1 className="absolute top-[140px] left-[-35px] w-[464px] text-center font-display text-[40px] leading-[0.86] tracking-[-0.03em] text-brand-dark-green md:left-1/2 md:w-[640px] md:-translate-x-1/2 md:text-[56px]">
          {t('hero.title')}
        </h1>
      ...
      <SectionShell className="grid gap-10 py-10 md:grid-cols-12 md:gap-3 md:py-20">
        ...
        <div className="divide-y divide-brand-dark-green/15 border-y border-brand-dark-green/15 md:col-span-6 md:col-start-7">
          {stackItems.map((item) => (
            <article key={item.title} className="grid gap-5 py-6 md:grid-cols-6">
              <Image
                src={item.icon}
                alt={item.alt}
                width={60}
