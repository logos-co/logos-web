...
const SUPPLY_CHAIN_ID = 'transaction-supply-chain'
type SupplyChainCopy = PrifiCopySection['supplyChain']

export function SupplyChain({ copy }: { copy: SupplyChainCopy }) {
  return (
    <section
      id={SUPPLY_CHAIN_ID}
    ...
    >
      ...
      <ContentWidth className="relative">
        <Intro copy={copy} />
        <ChainExplorer copy={copy} />
      </ContentWidth>
    </section>
  )
}

function Intro({ copy }: { copy: SupplyChainCopy }) {
  return (
    <div className="relative mx-auto flex max-w-[1140px] flex-col items-center gap-12 lg:flex-row lg:justify-between lg:gap-10">
      ...
      <div className="flex flex-col gap-[51px] lg:w-[631px]">
        ...
        <div className="flex flex-col gap-10 font-sans font-medium leading-[1.4]">
          <p className="text-[18px] lg:w-[602px] lg:text-[22px]">{copy.lead}</p>
          <p className="text-xs leading-[1.4] lg:w-[476px]">{copy.note}</p>
        </div>
      </div>
    </div>
  )
