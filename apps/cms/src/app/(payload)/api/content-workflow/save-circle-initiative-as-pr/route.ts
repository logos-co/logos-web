...
/**
 * POST /api/content-workflow/save-circle-initiative-as-pr
 *
 * Body: `{ id: string }` — the Payload CircleInitiative document id.
 *
...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
  ...
  let doc: CircleInitiativeDocLike
  try {
    doc = (await payload.findByID({
      collection: 'circle-initiatives',
      id: body.id,
      depth: 0,
    })) as unknown as CircleInitiativeDocLike
  } catch (err) {
    return NextResponse.json(
      {
        error: `failed to load CircleInitiative ${body.id}: ${
          err instanceof Error ? err.message : String(err)
        }`,
      },
      { status: 404 }
  ...
  try {
    const result = await saveCircleInitiativeAsPullRequest({
      doc,
      payload,
      editor: {
