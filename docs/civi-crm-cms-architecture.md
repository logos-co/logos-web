...
`civi-crm` is a Next.js application that provides a lightweight read/write interface into CiviCRM for Logos Circle Case management. It lives in `apps/civi-crm` inside the existing mono-repo, alongside `apps/web` and `apps/cms`.
...
All CMS-created activity entries use `activity_type_id: 80`.
...
| 5 | `activity_type_id` for CMS entries | **80** (§9.2). |
