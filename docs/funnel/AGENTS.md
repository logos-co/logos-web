...
| `packages/funnel/src/form-options.ts` | `SKILLS_OPTIONS` / `CHAT_SERVICE_OPTIONS` / `COUNTRY_OPTIONS` -- the dropdown options the three forms share (logos-web#140) |
...
- **One option list, not three** -- `skills`, `chatService` and `country` were byte-identical in all three form definitions, so they live once in `@repo/funnel` and reach the pages through `AFFORM_OPTIONS` (`apps/web/lib/funnel-forms/afform-options.ts`). The forms are not expected to diverge; a form that ever needs its own list can still put `options` on the field itself, which `getOptionsForField` prefers. The ids are the ids `apps/civi-crm` maps back to labels (`SKILLS_MAP` / `CHAT_SERVICE_MAP` / `COUNTRY_MAP`), and those maps are still a separate hand-maintained copy.
