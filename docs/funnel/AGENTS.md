...
- **One option list, read by both sides** -- `skills`, `chatService` and `country` were byte-identical in all three form definitions *and* duplicated a fourth time as the intake endpoint's id → label maps. They live once in `packages/funnel/src/form-options.ts`: `apps/web` reaches them through `AFFORM_OPTIONS` (`lib/funnel-forms/afform-options.ts`), and the endpoint's `SKILLS_MAP` / `CHAT_SERVICE_MAP` / `COUNTRY_MAP` are `toLabelMap()` over the same arrays, the way `HEAR_ABOUT_MAP` already was. An id can no longer mean one thing in the dropdown and another in Notion. The three forms are not expected to diverge; one that ever needs its own list can still put `options` on the field itself, which `getOptionsForField` prefers (logos-web#140).
